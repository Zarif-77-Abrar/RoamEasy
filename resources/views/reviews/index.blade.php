        ...
        @if(auth()->user()->hasRole('tourist'))
            <div class="bg-gray-100 rounded p-4 mt-6">
                ...
                <form action="{{ route('reviews.store', $destination->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label>Comment:</label>
                        <textarea name="comment" rows="3" required class="border p-2 rounded w-full"></textarea>
                    </div>
                    <button type="submit" class="bg-green-500 text-black px-4 py-2 rounded">Submit Review</button>
                </form>
            </div>
        @endif
    </div>
